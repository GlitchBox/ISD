...
class TFController {
  ...
  Future<List<TF>> getTFList(String token, int topicId) async {
    ...
    TFList tfList;
    if (count == 0) //no tf related to topic
    {
        tfList=await tfRest.getAllTF(token,topicId).catchError((Object onError){
          print(onError.toString());
          print("Pour some sugar on me");
        });
        await _insertTFList(tfList);
        return tfList.tfs;
    }
    List<TF> result = await tfDatabaseHelper.getTFList(topicId);
    return result;
  }
