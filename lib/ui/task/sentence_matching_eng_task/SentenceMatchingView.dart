...
class SentenceMatchingEngView extends StatefulWidget {
...
class _SMEState extends State<SentenceMatchingEngView>
  ...
  Offset moveValue(card, MainModel model) {
    if (_cards.indexOf(card) == 0 && model.smIsPressed)
      return _moveOutOfTheScreen.value;
    return Offset(0.0, 0.0);
  }

  double rotateValue(card, MainModel model) {
    if (_cards.indexOf(card) == 0 && model.smIsPressed) return _rotate.value;
    return 0.0;
  }
    ...
    smc.getSMList(model.user.token, model.currentTopic.id).then((qsList) {
      setState(() {
        ...
        for (int i = 0; i < jsonSetArrived; i++) {
          ...
          _cards[i] = new Map<String, String>();
        }

        int jsonNo = 0, index;
        qsList.forEach((qs) {
          index = (jsonNo ~/ 3);
          _cards.elementAt(index)[qs.banglaSentence] = qs.englishSentence;
