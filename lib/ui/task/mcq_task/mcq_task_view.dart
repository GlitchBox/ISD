...
class _MCQViewState extends State<MCQView> with TickerProviderStateMixin {
  ...
  void getQuestionsHardCode(){
    val=new List();
    isLoading=false;
    List<String>optionTemp=["Rajshahi","Dhaka","Sylhet","Chittagong"];
    List<String>correctTemp=["Dhaka"];
    MCQ mcq=new MCQ("What is the capital city of Bangladesh ?",optionTemp,correctTemp);
    val.add(mcq);

    List<String>optionTemp1=["Kuakata","Cox's Bazar","Both","None"];
    List<String>correctTemp1=["Cox's Bazar"];
    MCQ mcq1=new MCQ("What is the largest sea beach of Bangladesh ?",optionTemp1,correctTemp1);
    val.add(mcq1);

    List<String>optionTemp2=["Africa","Australia","Asia","Europe"];
    List<String>correctTemp2=["Asia"];
    MCQ mcq2=new MCQ("On which continent is Bangladesh located ?",optionTemp2,correctTemp2);
    val.add(mcq2);

     List<String>optionTemp3=["Sylhet","Dhaka","Chittagong","Khulna"];
    List<String>correctTemp3=["Sylhet"];
    MCQ mcq3=new MCQ("Which district of Bangladesh was part of Assam ?",optionTemp3,correctTemp3);
    val.add(mcq3);

     List<String>optionTemp4=["Dhaka Shangshad","Jatiyo Shangshad","Bangladesh Shangshad","None"];
    List<String>correctTemp4=["Jatiyo Shangshad"];
    MCQ mcq4=new MCQ("What is the name of national parliament of Bangladesh ?",optionTemp4,correctTemp4);
    val.add(mcq4);
                  ...
                  if (timerString != "0:00") {
                    animationController.forward().whenComplete(() {
                      setState(() {
                        ...
                        animationController.reset();
                        if (isLoading == false)
                          MCQ removedCard = model.getMCQVal.removeAt(0);
        ...
        if (model.getMCQVal.indexOf(car) < length) {
          return (Transform.translate(
              ...
              child: Transform.rotate(
                  ...
                  child: MCQStats(car, getAnswer, isLoading))));
        }
      }).toList());
    }
  }
  ...
  Offset moveValue(card,MainModel model) {
    if (model.getMCQVal.indexOf(card) == 0 && isPressed) return _moveOutOfTheScreen.value;
      ...
      setState(() {
       ...
       // List<String> correctAns = new List();
        ...
        for (int i = 0; i < qsList.length; i++) {
          ...
          correctAns.add(qsList.elementAt(i).answer);
          model.getMCQVal.add(new MCQ(qsList.elementAt(i).question, options, correctAns));
